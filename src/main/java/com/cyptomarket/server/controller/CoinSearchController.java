...
import com.cyptomarket.server.dto.CoinSearchRequestDto;
import com.cyptomarket.server.dto.CoinSearchResponseDto;
...
public class CoinSearchController {
    ...
    @PostMapping("/search")
    public ResponseEntity<List<CoinSearchResponseDto>> searchCoins(
            @Parameter(description = "검색 요청 객체") @RequestBody CoinSearchRequestDto request) {
        // Mock: 실제 검색 로직 대신 하드코딩된 결과 반환
        // 유효성 검사 예시: if (request.getKeyword().isEmpty()) throw new IllegalArgumentException("빈 검색어");
        CoinSearchResponseDto btc = new CoinSearchResponseDto(
